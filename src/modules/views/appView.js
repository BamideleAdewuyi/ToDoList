...
class AppView {
  constructor() {
    ...
    this.closeTaskFormButton = this.taskForm.querySelector(
      ".closeToDoFormButton",
    );
    this.projectForm = this.projectView.createProjectForm();
    this.closeProjectFormButton = this.projectForm.querySelector(
  ...
  addListeners() {
    ...
    this.projectForm.addEventListener("submit", (e) => {
      ...
      const project = this.projectView.projectController.createProject(
        title,
        this.projectView.projectController.id,
      );
    ...
    this.taskForm.addEventListener("submit", (e) => {
      ...
      const projectId = Number(
        this.getSelectedOption(document.getElementById("taskProject")).id,
      );
      const newTask = this.taskView.taskController.createTask(
        title,
        description,
        dueDate,
        priority,
        projectId,
        this.taskView.taskController.id,
      );
      const project = this.projectView.projectController.getProject(projectId);
      this.taskView.taskController.addTask(
        newTask,
        this.taskView.taskController.tasks,
      );
      this.projectView.projectController.refreshProjects(
        this.taskView.taskController,
      );
    ...
    for (const task of project.tasks) {
      ...
      detailsButton.addEventListener("click", () => {
        ...
        this.submitEditedTask(
          editTaskForm.form,
          editTaskForm.editButton,
          editTaskForm.closeButton,
          editTaskForm.task,
          editTaskForm.deleteTaskButton,
        );
      });
  ...
  submitEditedTask(taskForm, editButton, closeButton, task, deleteTaskButton) {
    ...
    editButton.addEventListener("click", (e) => {
      ...
      const dueDate = format(
        taskForm.querySelector("#taskDueDate").value,
        "dd/MM/yyyy",
      );
      ...
      const projectId = Number(
        this.getSelectedOption(taskForm.querySelector("#taskProject")).id,
      );
      const project = this.projectView.projectController.getProject(
        task.projectId,
      );
